...
//
// Основная логика подсчёта суммы живёт в SQL, поэтому заглушками её не
// проверить: пересечение периодов, число месяцев и семантика end_date IS NULL
// считаются самой базой. Тесты поднимают настоящий PostgreSQL в контейнере и
// накатывают на него те же миграции, что и сервис при старте.
//
// Если docker недоступен, тесты пропускаются: `go test ./...` должен
// отрабатывать и без него.
...
// Контейнер поднимается один на пакет: запускать PostgreSQL на каждый тест
// слишком дорого, а изоляцию даёт очистка таблицы в newRepository.
// Пустой skipReason означает, что база готова.
var (
...
func runTests(m *testing.M) (int, error) {
	...
	container, err := tcpostgres.Run(ctx, "postgres:16-alpine",
		...
		testcontainers.WithWaitStrategy(
			// Postgres в образе стартует дважды: сначала для initdb, потом
			// на приём соединений. Ждём второе сообщение, иначе подключение
			// оборвётся на середине инициализации.
