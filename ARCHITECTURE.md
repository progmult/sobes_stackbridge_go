...
1. **Наружу не уходят внутренние детали.** Имена Go-структур, их полей и
   ...
   Санировать ответ — не значит терять диагноз. Тип `clientError` хранит два
   представления одной ошибки: публичную формулировку и исходную причину.
   Клиент получает первую, лог — обе, причину отдельным атрибутом `cause`.
   `Unwrap` возвращает обе, поэтому `errors.Is` находит sentinel как обычно.
...
Хранилище заглушками не проверить: расчёт суммы за период — это SQL, а не Go.
Пересечение срока подписки с периодом, число месяцев в нём и семантика
`end_date IS NULL` считаются самой базой, поэтому его тесты поднимают
одноразовый Postgres через `testcontainers-go` и накатывают на него те же
миграции, что и сервис при старте — заодно проверяя, что схема ложится на
чистую базу.

Контейнер один на пакет: изоляцию между тестами даёт `TRUNCATE` перед каждым, а
не новая база — так прогон остаётся секундным. Если docker недоступен, тесты
пакета пропускаются, и `go test ./...` проходит без него; недоступность docker
отличается от сбоя контейнера явной проверкой в `TestMain`, поэтому сломанный
контейнер тесты уронит, а не замаскирует пропуском.
...
| Нет CI | Вне ТЗ; `make test` и `make vet` дают то же локально |
