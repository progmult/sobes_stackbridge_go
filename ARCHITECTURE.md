...
ошибок, а перечень уходит в поле `details` ответа.

Порядок при этом двухступенчатый: сначала разбор формата в `rest`, и только на
разобранных значениях — инварианты в `model`. Проверять инварианты того, что не
удалось разобрать, нечего.
